...
echo "## S35 -- like-for-like matrix, plain transformer   $(date -Iseconds)   $M"
...
echo
echo "   (no MTP rows: gemma-4 has no nextn_predict_layers, so draft-mtp is unavailable)"
