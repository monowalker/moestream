...
cd "$(dirname "$0")/../../.."
set -a; . ./.env; set +a
M=${M:-Ornith-1.0-35B-UD-IQ4_NL.gguf}
NAME=ms-s36; PORT=18114
...
cell() {  # $1=label $2=stream-env $3=spec $4=K
  ...
  docker run -d --name "$NAME" --entrypoint /usr/local/bin/moestream-entrypoint \
    ...
    -e MODEL_FILE="$M" -e CTX_SIZE=8192 -e UBATCH=1024 -e N_PARALLEL="$4" \
...
echo "## S36 -- like-for-like matrix, MoE   $(date -Iseconds)   $M"
...
cell "expert streaming"             "-e MOESTREAM=1 -e MOESTREAM_CACHE_FRAC=0.25" "" 1
echo "-- no MTP, four sequences ----------------------------------------------"
cell "plain llama.cpp, K=4"            "-e MOESTREAM=0" "" 4
cell "expert streaming, K=4"        "-e MOESTREAM=1 -e MOESTREAM_CACHE_FRAC=0.25" "" 4
echo
echo "   (no MTP rows: Ornith-1.0 has no nextn_predict_layers)"
